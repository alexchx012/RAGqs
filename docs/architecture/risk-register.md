...
Risk: settings now expose typed groups over the existing environment variables; the FastAPI app factory, provider factory, health checks, CORS options, knowledge retrieval tool, upload policy, ingestion storage runtime paths, document splitter, embedding/vector-store compatibility wrappers, Milvus client manager, and `RagAgentService` consume those grouped views; production deployment guardrails reject unsafe local defaults; real Milvus and Postgres smoke commands exist; and Milvus host ports are configurable for Windows reserved-port cases. Remaining direct flat config reads are concentrated in logger setup.

Mitigation: keep grouped settings views for app, CORS, uploads, deployment environment, providers, storage, agent, model providers, Milvus, RAG, and chunking. Keep the FastAPI app factory, uvicorn options, provider factory, health checks, CORS options, knowledge retrieval top-k, upload/ingestion paths, document splitter chunking, embedding/vector-store construction, Milvus client settings, and agent-service settings injection covered by tests; continue migrating the remaining logger setup to grouped settings while keeping `.env.example` synchronized with `app/config.py`, run `scripts/run-integration-smoke.ps1 -Json` before local or staged deployments that depend on Milvus, and run `scripts/run-postgres-smoke.ps1 -RequireConfigured -Json` before deployments that depend on Postgres-backed runtime state.
