...
- `RagAgentService` routes `query()`, `query_with_trace()`, `query_stream()`, and `query_stream_with_trace()` through a default explicit graph when `AGENT_RUNTIME=explicit_graph`, and it can receive grouped agent/RAG settings for model name, prompt profile, runtime, top-k, enabled tools, and tool-planning exclusions.
