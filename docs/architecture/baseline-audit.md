...
- Settings now expose typed groups over the existing `.env` variables, and the provider factory, health checks, upload/ingestion paths, knowledge retrieval tool, document splitter, embedding/vector-store compatibility wrappers, Milvus client manager, and `RagAgentService` consume grouped settings before falling back to flat fields. Some runtime modules still read flat global config fields directly.
