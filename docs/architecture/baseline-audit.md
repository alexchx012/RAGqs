...
- Settings now expose typed groups over the existing `.env` variables, and the provider factory, upload/ingestion paths, and `RagAgentService` consume grouped settings before falling back to flat fields. Some lower-level runtime modules still read flat global config fields directly.
