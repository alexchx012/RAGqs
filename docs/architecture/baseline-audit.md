...
- `app/main.py`: settings-aware FastAPI app factory, static file mount, router registration, Milvus startup check, and background indexing worker lifespan.
...
- Settings now expose typed groups over the existing `.env` variables, and the FastAPI app factory, provider factory, health checks, CORS options, logger setup, upload/ingestion paths, knowledge retrieval tool, document splitter, embedding/vector-store compatibility wrappers, Milvus client manager, and `RagAgentService` consume grouped settings before falling back to flat fields.
...
- Indexing defaults to synchronous execution and has retry, idempotent document ids, optional SQLite/Postgres job state, optional SQLite/Postgres document catalog storage, memory/SQLite/Postgres queue providers, pending-job recovery on worker startup, and an in-process background worker mode for queued uploads.
...
- Agent orchestration now defaults to explicit `StateGraph` with answer↔tool continuation and token streaming; SQLite, memory, and Postgres checkpoint providers sit behind `CHECKPOINT_PROVIDER`.
...
`app/services/vector_embedding_service.py` is now a lazy compatibility wrapper instead of constructing the DashScope client at import time, and it can build the DashScope embedding provider from grouped DashScope settings. `app/services/document_splitter_service.py` can receive grouped chunking settings for alternate ingestion factories. `app/core/milvus_client.py` and `app/services/vector_store_manager.py` can receive grouped Milvus settings, while the vector store manager still delegates actual vector-store operations to `MilvusVectorStoreProvider`. `app/tools/knowledge_tool.py` now retrieves through `RetrieverProvider`, applies grouped RAG top-k settings before flat fallback, and gives agents and future evaluators the same retrieval contract. `app/api/file.py` now sends upload indexing through `ProviderContainer.ingestion_provider` while preserving upload security validation and the existing indexing-status response.

Current Phase 1 test coverage includes grouped settings, provider protocols, fake providers, lazy DashScope/Milvus construction, provider container wiring from grouped model/RAG/Milvus/storage settings, grouped Milvus settings for client/vector-store compatibility managers, grouped RAG top-k settings for the knowledge retrieval tool, grouped chunking settings for document splitting, grouped DashScope embedding settings for the compatibility builder, grouped app/CORS settings for the FastAPI app factory, uvicorn options, and logger setup, grouped health-check settings with injected dependency probes, `RagAgentService` grouped agent/RAG settings injection, `RagAgentService` chat model injection, provider-backed knowledge retrieval, upload indexing through the ingestion provider, grouped upload security settings, grouped ingestion storage settings, service-side session store injection, SQLite/Postgres session persistence, Postgres indexing-job persistence, Postgres document catalog persistence, and Postgres checkpoint selection through injected factories. Remaining provider work is mainly production collection/knowledge-space selection.
...
- `queue.py`: indexing queue protocol plus in-memory FIFO, SQLite-backed, and Postgres-backed implementations with job deduplication.
...
- `rag_graph.py`: explicit LangGraph `StateGraph` builder with typed state and `normalize_input`, `decide_retrieval`, `retrieve`, `handoff`, `tool`, `answer`, `error_policy`, and `final_response` nodes plus `ChatModelAnswerGenerator` and `LangChainToolExecutor`.
- The graph accepts injectable `RetrieverProvider`, `AnswerGenerator`, `ToolExecutor`, and LangGraph checkpointer implementations, propagates sources and retrieval debug data into state, accumulates routing/retrieval/tool/handoff/answer/error-policy/final events, resets per-run transient events across checkpointed invocations, and can compile with memory, SQLite, or Postgres checkpoint providers.
- `RagAgentService` routes `query()`, `query_with_trace()`, `query_stream()`, and `query_stream_with_trace()` through a default explicit graph when `AGENT_RUNTIME=explicit_graph`, and it can receive grouped agent/RAG settings for model name, prompt profile, runtime, top-k, and enabled tools.
- There is no pre-retrieval planner. `decide_retrieval` routes to tools only for an explicit `tool_request`; during answer generation the model may emit `tool_calls` and the graph continues `answer ↔ tool` until a final answer or tool-round limit.
- Streaming explicit-graph calls use LangGraph `stream_mode=["custom", "updates"]`; `ChatModelAnswerGenerator.stream_ai_message()` emits content-only token chunks through custom stream events while assembled AIMessages carry tool_calls for routing; node updates carry retrieval, tool, error, and final `done` events.
...
- `ChatRequest.spaceId`, `RagAgentService.retrieve_context(..., space_id=...)`, and `retrieve_knowledge(..., space_id=...)` pass knowledge-space filters into retrieval; request-scoped enforcement still overrides tool-call arguments.
...
- `app/operations/health.py`: composable dependency health checks with explicit `app`, `modelProvider`, `embeddingProvider`, `vectorStore`, and `sessionStore` boundaries; the default checker accepts grouped settings plus injected Milvus/session dependencies for tests and alternate deployments.
...
- `app/extensions/tools.py`: ordered tool registry for built-in and business-specific LangChain tools, with `ENABLED_TOOLS` selecting the runtime tool list used by answer↔tool continuation and explicit `tool_request` execution.
