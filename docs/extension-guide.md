...
Use `fake` providers for local tests and demos that must not call DashScope, Milvus, or external APIs. Use `openai_compatible` for chat or embedding endpoints that implement OpenAI-compatible APIs. Install the optional `postgres` dependency group before enabling Postgres-backed session, indexing queue, indexing job, document catalog, or checkpoint storage. For multi-instance ingestion, set the queue, indexing job, and document catalog providers to Postgres so API and worker processes share the same job ownership state.
