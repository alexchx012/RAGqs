...
- Create `.env` from `.env.example`. Default hybrid providers need:
  - a real `DEEPSEEK_API_KEY` for chat (with `CHAT_MODEL=deepseek-v4-pro`), and
  - a real `DASHSCOPE_API_KEY` for embedding (with `DASHSCOPE_EMBEDDING_MODEL=text-embedding-v4`).
- Leave `CHAT_PROVIDER` blank for auto-selection. DeepSeek-first ordering applies only when two valid
  chat keys are present; a single valid chat key is selected without priority comparison. DashScope
  chat, OpenAI-compatible chat, and `fake` require an explicit `CHAT_PROVIDER`.
...
Default sample chat/embedding settings:
...
or `documentCatalog`. Health and readiness only validate configuration and
dependency boundaries (`CHAT_MODEL` + selected chat key, DashScope embedding key
and model, store providers). They do not prove a real DeepSeek chat call or
business answer quality; real DeepSeek smoke is a separate opt-in gate.

The smoke gate checks configuration and Milvus without creating, deleting,
starting, stopping, or restarting Milvus.
...
The report is uploaded as the `evaluation-report` artifact. The hosted workflow intentionally skips `start.ps1 -PreflightOnly` because CI does not provide local DeepSeek / DashScope keys or a Milvus stack by default.

## Rollback

This provider-default change is a code and sample-config rollback only. Revert the
commits from the DeepSeek chat adoption change and re-run baseline validation.
There is no vector index, database schema, or public API data migration, so do not
plan a data rollback when restoring the previous defaults.
