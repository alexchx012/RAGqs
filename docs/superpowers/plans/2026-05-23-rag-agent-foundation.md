...
Current progress: `app/retrieval/` now provides a composable `RetrievalPipeline` that implements `RetrieverProvider` and wraps one or more retrievers with optional query rewrite, deduplication, rerank, context compression, citation extraction, and per-stage timing debug data. `ProviderContainer` now wires the default vector-store retriever through this pipeline, so `knowledge_tool` uses the new retrieval foundation by default. `RagAgentService.query_with_trace()` and `query_stream_with_trace()` expose sources and retrieval debug data through `/chat` and `/chat_stream`. This is an interim compatibility path before Phase 4 moves retrieval and answer generation into a single explicit `StateGraph`; production reranker/query-rewrite providers are still pending.
...
Current progress: Phase 5 now has server-side session recording, searchable session summaries, optional SQLite session persistence, optional Postgres session persistence, backend-first frontend history, process-local/SQLite/Postgres knowledge spaces/document catalog storage, optional SQLite/Postgres indexing job persistence, document lifecycle APIs, and request-scoped space enforcement for both direct retrieval and legacy tool retrieval. Remaining work includes production multi-instance checkpoint backends beyond local SQLite.
...
Current progress: Phase 8 has a tested extension layer for tool registration, optional model tool planning, prompt profile selection, provider switching, local test doubles, SQLite session/job/document/checkpoint storage, Postgres session/indexing-job/document storage, configurable agent runtime, and second-business documentation. Remaining foundation work after this plan includes deeper production capabilities such as multi-instance checkpoint storage and hosted CI wiring.
