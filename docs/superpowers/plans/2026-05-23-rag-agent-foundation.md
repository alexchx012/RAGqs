...
- [x] Keep existing API behavior unchanged while switching service construction to providers. Current progress: `RagAgentService` accepts injected grouped settings, `ChatModelProvider`, `SessionStoreProvider`, `CheckpointProvider`, custom agent factory, tools, checkpointer, and configurable `AGENT_RUNTIME`; `knowledge_tool` retrieves through `RetrieverProvider` and reads grouped RAG top-k settings; file upload indexing now runs through `ProviderContainer.ingestion_provider`; and `ProviderContainer` wires chat, embedding, vector store, retriever, session, ingestion, and checkpoint providers.

Current progress: `app/config.py` now exposes typed grouped settings for app, CORS, uploads, providers, storage, agent, OpenAI-compatible, DashScope, Milvus, RAG, and chunking configuration while preserving the existing flat `.env` names. `ProviderSelection`, startup config validation, `ProviderContainer` construction, the FastAPI app factory, CORS options, uvicorn options, logger setup, health checks, knowledge retrieval, and `RagAgentService` can consume these grouped settings. Upload security policy construction now reads `settings.upload`, ingestion job/catalog store construction now reads `settings.storage`, `DocumentSplitterService` reads grouped chunking settings, `vector_embedding_service` builds the DashScope embedding provider from grouped DashScope settings, `MilvusClientManager` and `VectorStoreManager` read grouped Milvus settings, `create_default_health_checker()` reads grouped app/RAG/DashScope/Milvus settings with injectable Milvus/session dependencies, `retrieve_knowledge_with_settings()` reads grouped RAG top-k settings, and `RagAgentService` reads grouped agent/RAG settings for runtime, prompt profile, enabled tools, tool-planning exclusions, model name, and retrieval top-k before falling back to flat fields. `PostgresSessionStoreProvider` adds the first multi-instance session backend behind `SESSION_STORE_PROVIDER=postgres` and `SESSION_STORE_POSTGRES_DSN`; `PostgresIndexingJobStore` adds multi-instance indexing status behind `INDEXING_JOB_STORE_PROVIDER=postgres` and `INDEXING_JOB_STORE_POSTGRES_DSN`; `PostgresKnowledgeCatalog` adds multi-instance document lifecycle metadata behind `DOCUMENT_CATALOG_PROVIDER=postgres` and `DOCUMENT_CATALOG_POSTGRES_DSN`; `PostgresCheckpointProvider` adds multi-instance graph checkpoint state behind `CHECKPOINT_PROVIDER=postgres` and `CHECKPOINT_POSTGRES_DSN`. Remaining configuration work is keeping future runtime modules on grouped settings and provider boundaries.

## Phase 2: Ingestion Foundation

- [x] Create document loader interfaces for uploaded TXT, Markdown, CSV, HTML/HTM, and JSON files.
...
- [x] Add optional background job execution for uploads. Current progress: `INDEXING_EXECUTION_MODE=background` creates pending jobs and enqueues them through an indexing queue boundary for a FastAPI lifespan worker; `INDEXING_QUEUE_PROVIDER=sqlite` keeps durable local queueing, `INDEXING_QUEUE_PROVIDER=memory` remains available for throwaway tests, `INDEXING_QUEUE_PROVIDER=postgres` plus `INDEXING_QUEUE_POSTGRES_DSN` enables shared queue claims for multi-instance ingestion, `INDEXING_WORKER_RECOVER_PENDING_JOBS=true` re-enqueues persisted pending jobs on startup, and `sync` remains the default compatibility mode.

Current progress: `app/ingestion/` now contains UTF-8 TXT, Markdown, CSV, HTML/HTM, and JSON loaders, metadata normalization, an indexing job state model, in-memory, SQLite, and Postgres job stores, an indexing queue protocol with in-memory FIFO, SQLite-backed, and Postgres-backed implementations, pending-job recovery, and an in-process background worker. `VectorIndexService` accepts loader, splitter, vector store, normalizer, job store, and document catalog dependencies; defaults local development to SQLite queue/job/catalog storage while still allowing memory/Postgres selection; deletes previous chunks by stable `document_id` and also clears legacy source-only chunks by source path; enriches chunks with normalized metadata; records successful and failed jobs; creates pending jobs for queued execution; and returns per-file job summaries for directory indexing. Upload responses include indexing status, synchronous indexing failures return HTTP 500 details, background indexing returns pending jobs, and `/index-jobs` APIs support list, detail, and retry. Remaining work includes production load testing and tuning for high-volume distributed ingestion.
...
- [x] Add a local evaluation command that can run with fake providers and optional real providers. Current progress: `scripts/run-evaluation.ps1` supports `fake`, in-process `service`, and external FastAPI `http` modes against configurable JSONL datasets, with `none`, `static`, and `model` faithfulness judge modes, per-example `metadata.spaceId` routing, configurable `-MinExamples` readiness thresholds, real-provider readiness checks through `-PreflightOnly`, and JSON reports through `-ReportPath`.
- [x] Document how to enable LangSmith tracing and evaluation when credentials are available.

Current progress: `app/evaluation/` now provides typed golden examples, run results, faithfulness verdicts, aggregate reports, JSONL loading, deterministic metric calculation, a `FaithfulnessJudge` boundary with static and model-backed implementations, a fake-provider runner, an async traced-service runner, provider-agnostic model judge selection, a space-aware HTTP `/chat` evaluation client, business golden dataset quality checks, and a real-provider readiness report that validates service/http mode, dataset strength, provider selection, model-judge credentials, HTTP targets, and LangSmith tracing settings before running expensive evaluations. `data/evaluation/business.example.jsonl` and `docs/business-samples/` provide a stronger space-scoped example workflow beyond the toy fake dataset. `scripts/run-evaluation.ps1` is included in baseline validation so the evaluation harness itself cannot silently regress, supports `-MinExamples` for stricter business gates, and writes CI-friendly JSON reports. `.env.example` and `docs/evaluation.md` document LangSmith tracing variables for real service evaluation.
...
- [x] Split health checks into app, model provider, embedding provider, vector store, and session store. Current progress: `/health` uses a composable `HealthChecker`, reports `app`, `modelProvider`, `embeddingProvider`, `vectorStore`, and `sessionStore` dependency states, and the default checker can receive grouped settings plus injected Milvus/session dependencies.
...
Current progress: `app/observability/`, `app/operations/`, and `app/security/` provide the first operations boundaries for request tracing, structured access logs, in-process runtime metrics, Prometheus-compatible metrics export, retrieval audit persistence, dependency health, configuration validation, production deployment guardrails, running-API health preflight, non-destructive Milvus smoke checks, Postgres reachability and opt-in write-path smoke checks, configurable Milvus host ports for Windows reserved-port cases, CORS configuration, upload validation, prompt-injection screening, and shared JSON response envelope helpers. `app/main.py` now exposes a settings-aware FastAPI app factory, grouped app/CORS/uvicorn settings, injectable health checker dependencies, and grouped storage settings for background indexing lifespan behavior. `GET /api/metrics` exposes HTTP request counts, status codes, route latency, RAG success/failure counts, per-space RAG counts, latency buckets, and provider token usage when available; `GET /api/metrics/prometheus` exports the same signal as plain text for external scraping. `docs/operations.md` and `docs/deployment.md` document the runtime contract, including runtime metrics, retrieval audits, Docker profiles, health gates, integration smoke gates, Postgres smoke gates, security boundaries, hosted CI, and CI evaluation artifacts. Production secret management, central log/trace/metrics storage, and provider-specific Postgres schema validation remain open.
...
- [x] Add provider switching examples for DashScope, OpenAI-compatible APIs, and local test doubles. Current progress: provider ids are configurable through `CHAT_PROVIDER`, `EMBEDDING_PROVIDER`, `VECTOR_STORE_PROVIDER`, `SESSION_STORE_PROVIDER`, `INGESTION_PROVIDER`, and `INDEXING_QUEUE_PROVIDER`; `dashscope`, `openai_compatible`, `fake`, SQLite session, Postgres session, memory queue, SQLite queue, and Postgres queue provider paths are covered by tests and docs.
...
Current progress: Phase 8 has a tested extension layer for tool registration, optional model tool planning, prompt profile selection, provider switching, indexing execution mode, memory/SQLite/Postgres indexing queue selection, retrieval profile selection, retrieval enhancer switches, local test doubles, SQLite-by-default session/queue/job/document/checkpoint storage, Postgres session/indexing-queue/indexing-job/document/checkpoint storage, configurable agent runtime, and second-business documentation. Remaining foundation work after this plan includes deeper production capabilities such as real provider evaluation, production trace analysis, and high-volume distributed ingestion tuning.
...
- [x] Postgres indexing queue provider tests
...
- [x] Postgres smoke command for configured Postgres-backed stores, with default read-only reachability and opt-in temporary-table write-path validation
