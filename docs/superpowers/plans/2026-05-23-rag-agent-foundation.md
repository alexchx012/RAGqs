...
Current progress: `app/observability/`, `app/operations/`, and `app/security/` provide the first operations boundaries for request tracing, structured access logs, in-process runtime metrics, Prometheus-compatible metrics export, retrieval audit persistence, dependency health, configuration validation, production deployment guardrails, running-API health preflight, non-destructive Milvus and Postgres smoke checks, configurable Milvus host ports for Windows reserved-port cases, CORS configuration, upload validation, prompt-injection screening, and shared JSON response envelope helpers. `app/main.py` now exposes a settings-aware FastAPI app factory, grouped app/CORS/uvicorn settings, injectable health checker dependencies, and grouped storage settings for background indexing lifespan behavior. `GET /api/metrics` exposes HTTP request counts, status codes, route latency, RAG success/failure counts, per-space RAG counts, latency buckets, and provider token usage when available; `GET /api/metrics/prometheus` exports the same signal as plain text for external scraping. `docs/operations.md` and `docs/deployment.md` document the runtime contract, including runtime metrics, retrieval audits, Docker profiles, health gates, integration smoke gates, Postgres smoke gates, security boundaries, hosted CI, and CI evaluation artifacts. Production secret management, central log/trace/metrics storage, and real Postgres audit write-path validation remain open.
