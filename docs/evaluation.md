...
Use `-FaithfulnessJudge static` for the deterministic judge boundary. Use
`-FaithfulnessJudge model` with `-Mode service` or `-Mode http` to score answers
through the configured chat provider boundary (`CHAT_PROVIDER` optional + required
`CHAT_MODEL` + the selected provider key). Examples:

- DeepSeek (default candidate): `DEEPSEEK_API_KEY` + `CHAT_MODEL`
- DashScope chat: `CHAT_PROVIDER=dashscope` + `DASHSCOPE_API_KEY` + `CHAT_MODEL`
- OpenAI-compatible chat: `CHAT_PROVIDER=openai_compatible` +
  `OPENAI_COMPATIBLE_API_KEY` + optional `OPENAI_COMPATIBLE_BASE_URL` +
  `CHAT_MODEL`

`DASHSCOPE_EMBEDDING_MODEL` is embedding-only and is not a chat/judge model name.
There is no RAG-specific chat model variable; retrieval enhancers that need an LLM
reuse `ProviderContainer.chat_model_provider`.
...
do not treat them as real business quality approval. Configuration readiness and
health checks also stop at config completeness—they are not real DeepSeek smoke
or answer-quality gates.
...
`-Mode service` calls the in-process `rag_agent_service`, so it requires valid local configuration for the selected chat provider, DashScope embedding (default), Milvus, and indexed documents. `-Mode http` posts to `/chat` on a running FastAPI server and maps the response into the same evaluation result model. Use `-ReportPath artifacts\evaluation-report.json` to write a CI-friendly JSON report; product-grade trace analysis remains tied to the broader LangSmith and observability roadmap.
