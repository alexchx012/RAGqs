...
describe("admin contract：dashboard（§9.1）", () => {
  it("ops 组包：待办卡联动实时 pending 计数；api_latency 为无数据卡", async () => {
    ...
    const submissionCard = cards.find(
      (card) => card.key === "submission_pending",
    );
  ...
  it("backlog 是当前快照，事件卡仍按窗口变化", async () => {
    const ops = bearerOf("ops-wang");
    const today = (await (
      await jsonRequest(ops, "/v1/metrics/dashboard?window=today")
    ).json()) as DashboardResponse;
    const month = (await (
      await jsonRequest(ops, "/v1/metrics/dashboard?window=30d")
    ).json()) as DashboardResponse;
    const cardValue = (dashboard: DashboardResponse, key: string) => {
      const card = dashboard.packs
    ...
    expect(cardValue(today, "failure_rate")).not.toBe(
      cardValue(month, "failure_rate"),
    );
  });
  ...
  it("admin 组包：结构不同、threshold/link 恒 null；expand=user_rank 返回完整排行", async () => {
    ...
    const response = await jsonRequest(
      admin,
      "/v1/metrics/dashboard?window=7d&expand=user_rank",
    );
    expect(response.status).toBe(200);
    const body = (await response.json()) as DashboardResponse;
  ...
  it("其余角色 403；非法窗口 422", async () => {
    ...
    await expectError(
      await jsonRequest(user, "/v1/metrics/dashboard?window=7d"),
      403,
      "metrics_forbidden",
    );
    const ops = bearerOf("ops-wang");
    await expectError(
      await jsonRequest(ops, "/v1/metrics/dashboard?window=bogus"),
      422,
      "validation_error",
    );
  });
  ...
  it("指标看板（§9.2）：固定三卡无组包，窗口沿用", async () => {
    ...
    const response = await jsonRequest(
      ops,
      "/v1/metrics/operations?window=today",
    );
    expect(response.status).toBe(200);
...
describe("admin contract：approvals summary（§8.1）", () => {
  ...
  it("admin / 部长 / 普通用户：quota_pending 恒 0，投稿计数按角色范围", async () => {
    ...
    expect(
      (await (
        await jsonRequest(admin, "/v1/approvals/summary")
      ).json()) as ApprovalSummary,
    ).toEqual({
      quota_pending: 0,
    ...
    expect(
      (await (
        await jsonRequest(minister, "/v1/approvals/summary")
      ).json()) as ApprovalSummary,
    ).toEqual({
      quota_pending: 0,
    ...
    expect(
      (await (
        await jsonRequest(user, "/v1/approvals/summary")
      ).json()) as ApprovalSummary,
    ).toEqual({
      quota_pending: 0,
...
describe("admin contract：配额申请审批（§8.2–8.3）", () => {
  async function pendingQuotaIds(
    token: string,
  ): Promise<QuotaRequestListResponse> {
    const response = await jsonRequest(
      token,
      "/v1/approvals/quota-requests?status=pending",
    );
    expect(response.status).toBe(200);
  ...
  it("待办列表按申请时间正序；status=approved 过滤出历史", async () => {
    ...
    expect(pending.items.map((item) => item.applicant.id)).toEqual([
      "u_user",
      "u_minister",
      "u_ghost",
    ]);
    const approved = await jsonRequest(
      ops,
      "/v1/approvals/quota-requests?status=approved",
    );
  ...
  it("批准：200 + 默认页数；幂等回放同快照；同键不同体 409；通知联动", async () => {
    ...
    const notifications = (await (
      await jsonRequest(user, "/v1/notifications")
    ).json()) as {
      items: { type: string }[];
    };
    expect(
      notifications.items.some((item) => item.type === "quota_approved"),
    ).toBe(true);
  });
  ...
  it("批准错误系列：version_conflict / not_approvable / 非 ops 403 / 缺 key 422", async () => {
    const ops = bearerOf("ops-wang");
    const pending = await pendingQuotaIds(ops);
    const ministerRequest = pending.items.find(
      (item) => item.applicant.id === "u_minister",
    );
    const ghostRequest = pending.items.find(
      (item) => item.applicant.id === "u_ghost",
    );
  ...
  it("驳回：200 + 通知联动；请求体多字段 422", async () => {
    ...
    const minister = bearerOf("minister-li");
    const notifications = (await (
      await jsonRequest(minister, "/v1/notifications")
    ).json()) as {
      items: { type: string }[];
    };
    expect(
      notifications.items.some((item) => item.type === "quota_rejected"),
    ).toBe(true);

    const another = (await pendingQuotaIds(ops)).items.find(
      (item) => item.applicant.id === "u_ghost",
    );
    await expectError(
      await postWithKey(
...
describe("admin contract：投稿审核（§8.4–8.5）", () => {
  ...
  it("ops 只见公共库；admin 见公共库 + 全部部门库", async () => {
    ...
    expect(opsList.items.every((item) => item.space_id === "public")).toBe(
      true,
    );
  ...
  it("批准 202；重复审批 409 submission_already_reviewed", async () => {
    const ops = bearerOf("ops-wang");
    const target = (await listSubmissions(ops)).items.find(
      (item) => item.file_name === "行业研报汇总.pdf",
    );
    const approved = await postWithKey(
      ops,
    ...
    expect(((await approved.json()) as ApprovalDecisionResponse).status).toBe(
      "approved",
    );
    await expectError(
      await postWithKey(
  ...
  it("duplicate_document 行不移除；scope_changed 行失效；冻结投稿人 409", async () => {
    ...
    const duplicate = items.find(
      (item) => item.file_name === "公共制度汇编.pdf",
    );
    await expectError(
      await postWithKey(
        ops,
    ...
    expect(
      (await listSubmissions(ops)).items.some(
        (item) => item.submission_id === duplicate?.submission_id,
      ),
    ).toBe(true);

    const scopeChanged = items.find(
      (item) => item.file_name === "跨部门协作指引.pdf",
    );
    await expectError(
      await postWithKey(
        ops,
    ...
    expect(
      (await listSubmissions(ops)).items.some(
        (item) => item.submission_id === scopeChanged?.submission_id,
      ),
    ).toBe(false);
...
describe("admin contract：图谱构建（§6.12）", () => {
  ...
  it("202 创建 → 轮询推进至 succeeded → availability ready + 终态通知发起者", async () => {
    ...
    expect(succeeded.latest_run).toMatchObject({
      state: "succeeded",
      version: 3,
    });
    ...
    expect(((await replay.json()) as GraphBuildRun).graph_build_id).toBe(
      run.graph_build_id,
    );

    const notifications = (await (
      await jsonRequest(ops, "/v1/notifications")
    ).json()) as {
    ...
    expect(
      notifications.items.some(
        (item) =>
          item.type === "graph_build_completed" &&
          item.payload["status"] === "succeeded",
      ),
    ).toBe(true);
  });
  ...
  it("source_revision 不一致 409；预估不可用 503；公共库为空 422", async () => {
    ...
    const docs = (await (
      await jsonRequest(ops, "/v1/spaces/public/documents?page=1&page_size=10")
    ).json()) as {
  ...
  it("取消：200 + 重复取消返回终态；终态后取消 409；不存在 404", async () => {
    ...
    expect(((await again.json()) as GraphBuildCancelResponse).state).toBe(
      "cancelled",
    );
    ...
    await expectError(
      await postWithKey(
        ops,
        "/v1/ops/graph-builds/gb_nope/cancel",
        { expected_version: 1 },
        "idem_g_9",
      ),
      404,
...
describe("admin contract：任务队列（§10.1–10.2）", () => {
  it("四档视图与 stale_count；ops 行带操作、admin 行空数组", async () => {
    const ops = bearerOf("ops-wang");
    const all = (await (
      await jsonRequest(ops, "/v1/ops/jobs?view=all")
    ).json()) as OpsJobsResponse;
    expect(all.items.length).toBe(9);
    expect(all.stale_count).toBe(2);
    const active = (await (
      await jsonRequest(ops, "/v1/ops/jobs?view=active")
    ).json()) as OpsJobsResponse;
    expect(active.items.length).toBe(5);
    // replayable（待人工处理）= failed + dead_letter + cancelled（§10.1 视图定义）
    const replayable = (await (
      await jsonRequest(ops, "/v1/ops/jobs?view=replayable")
    ).json()) as OpsJobsResponse;
    expect(replayable.items.length).toBe(3);
    const stale = (await (
      await jsonRequest(ops, "/v1/ops/jobs?view=stale")
    ).json()) as OpsJobsResponse;
    ...
    const running = all.items.find(
      (job) => job.state === "running" && !job.stale,
    );
    ...
    const adminAll = (await (
      await jsonRequest(admin, "/v1/ops/jobs?view=all")
    ).json()) as OpsJobsResponse;
    expect(
      adminAll.items.every((job) => job.allowed_actions.length === 0),
    ).toBe(true);
  ...
  it("普通用户 403；非法 view 422；cancel 无 key 204；replay 202 语义", async () => {
    const user = bearerOf("zhangsan");
    await expectError(
      await jsonRequest(user, "/v1/ops/jobs?view=all"),
      403,
      "ops_jobs_forbidden",
    );
    const ops = bearerOf("ops-wang");
    await expectError(
      await jsonRequest(ops, "/v1/ops/jobs?view=bogus"),
      422,
      "validation_error",
    );
    ...
    const cancel = await jsonRequest(
      ops,
      `/v1/ingestion-jobs/${pending?.job_id}/cancel`,
      { method: "POST" },
    );
    ...
    expect(
      afterCancel.items.find((job) => job.job_id === pending?.job_id)?.state,
    ).toBe("cancelled");

    const failed = all.items.find((job) => job.state === "failed");
    const replay = await postWithKey(
      ops,
      `/v1/ingestion-jobs/${failed?.job_id}/replay`,
      undefined,
      "idem_j_1",
    );
    expect(replay.status).toBe(200);
    ...
    expect(
      afterReplay.items.find((job) => job.job_id === failed?.job_id)?.state,
    ).toBe("pending");
  });
});
...
describe("admin contract：评测与校准（§11）", () => {
  it("leaderboard：正式 3 行 / 影子 2 行 / 策略版本；普通用户 403", async () => {
    const ops = bearerOf("ops-wang");
    const body = (await (
      await jsonRequest(ops, "/v1/evaluation/leaderboard")
    ...
    const user = bearerOf("zhangsan");
    await expectError(
      await jsonRequest(user, "/v1/evaluation/leaderboard"),
      403,
  ...
  it("无窗口时合成 closed 读模型", async () => {
    const ops = bearerOf("ops-wang");
    const body = (await (
      await jsonRequest(ops, "/v1/calibration/window")
    ).json()) as CalibrationWindow;
    expect(body).toEqual({
      window_id: null,
      status: "closed",
      opened_at: null,
  ...
  it("开窗 201 / 状态机 409 系列 / 关窗 200 closing", async () => {
    ...
    expect(((await replay.json()) as CalibrationWindow).window_id).toBe(
      openedBody.window_id,
    );
    // 同键不同体：409
    await expectError(
      await postWithKey(
    ...
    const closed = await postWithKey(
      ops,
      "/v1/calibration/window",
      { action: "close" },
      "idem_c_4",
  ...
  it("closed 状态关窗 409 not_open；资格不足 409 not_eligible（manual 豁免）", async () => {
    ...
    await expectError(
      await postWithKey(
        ops,
        "/v1/calibration/window",
        { action: "open", window_kind: "sentinel" },
        "idem_c_7",
      ),
      409,
...
describe("admin contract：用户管理（§12.1–12.4）", () => {
  it("目录：过滤 / 分页 / 角色可见性", async () => {
    const admin = bearerOf("admin");
    const all = (await (
      await jsonRequest(admin, "/v1/admin/users?page=1&page_size=20")
    ).json()) as AdminUserListResponse;
    expect(all.total).toBe(9);

    const byQuery = (await (
      await jsonRequest(admin, "/v1/admin/users?q=zhangsan")
    ).json()) as AdminUserListResponse;
    expect(byQuery.items.map((item) => item.username)).toEqual(["zhangsan"]);

    const byDepartment = (await (
      await jsonRequest(admin, "/v1/admin/users?department_id=d_finance")
    ).json()) as AdminUserListResponse;
    expect(byDepartment.total).toBe(2);

    const byRole = (await (
      await jsonRequest(admin, "/v1/admin/users?role=user")
    ).json()) as AdminUserListResponse;
    expect(byRole.total).toBe(4);

    const page = (await (
      await jsonRequest(admin, "/v1/admin/users?page=3&page_size=3")
    ).json()) as AdminUserListResponse;
    expect(page).toMatchObject({ total: 9, page: 3, page_size: 3 });
    expect(page.items.length).toBe(3);

    await expectError(
      await jsonRequest(admin, "/v1/admin/users?role=bogus"),
      422,
      "validation_error",
    );
    const ops = bearerOf("ops-wang");
    ...
    const user = bearerOf("zhangsan");
    await expectError(
      await jsonRequest(user, "/v1/admin/users"),
      403,
  ...
  it("创建 201 与 409/422/404/403 系列", async () => {
    ...
    const created = await jsonRequest(admin, "/v1/admin/users", {
      method: "POST",
      idempotencyKey: "idem_user_create_1",
      body: {
        username: "newhire",
        real_name: "New Hire",
        department_id: "d_hr",
    ...
    await expectError(
      await jsonRequest(admin, "/v1/admin/users", {
        method: "POST",
        idempotencyKey: "idem_user_k_2",
        body: { ...base, username: "zhangsan" },
      }),
      409,
      "username_exists",
    );
  ...
  it("编辑：版本冲突 / 显式 null 解绑 / 目标守卫三态", async () => {
    ...
    const patched = await jsonRequest(admin, "/v1/admin/users/u_chen", {
      method: "PATCH",
      idempotencyKey: "idem_user_patch_1",
      body: { expected_version: 1, role: "minister", department_id: "d_hr" },
    });
    expect(patched.status).toBe(200);
    expect((await patched.json()) as AdminUserItem).toMatchObject({
      role: "minister",
      version: 2,
    });
    ...
    const unbound = await jsonRequest(admin, "/v1/admin/users/u_chen", {
      method: "PATCH",
      idempotencyKey: "idem_user_patch_2",
  ...
  it("永久禁用 202：pending_delete + purge_after_at；重复禁用 409", async () => {
    ...
    const deleted = await jsonRequest(admin, "/v1/admin/users/u_zhao", {
      method: "DELETE",
      idempotencyKey: "idem_user_delete_1",
      body: { expected_version: 1 },
    });
...
describe("admin contract：部门管理（§12.5–12.6）", () => {
  it("目录：三档过滤与 allowed_actions 角色差异", async () => {
    const admin = bearerOf("admin");
    const active = (await (
      await jsonRequest(admin, "/v1/admin/departments?status=active")
    ).json()) as AdminDepartmentListResponse;
    expect(active.items.length).toBe(3);
    expect(
      active.items.every((item) => item.allowed_actions.length === 2),
    ).toBe(true);
    const inactive = (await (
      await jsonRequest(admin, "/v1/admin/departments?status=inactive")
    ).json()) as AdminDepartmentListResponse;
    expect(inactive.items.map((item) => item.name)).toEqual(["档案部"]);
    expect(inactive.items[0]?.allowed_actions).toEqual([]);
    const all = (await (
      await jsonRequest(admin, "/v1/admin/departments?status=all")
    ).json()) as AdminDepartmentListResponse;
    expect(all.items.length).toBe(4);

    const ops = bearerOf("ops-wang");
    const opsList = (await (
      await jsonRequest(ops, "/v1/admin/departments?status=all")
    ).json()) as AdminDepartmentListResponse;
    expect(
      opsList.items.every((item) => item.allowed_actions.length === 0),
    ).toBe(true);
    await expectError(
  ...
  it("创建 201 + 幂等回放 + 重名 409 + 运维 403", async () => {
    ...
    expect(((await replay.json()) as AdminDepartmentItem).id).toBe(
      createdBody.id,
    );
    const all = (await (
      await jsonRequest(admin, "/v1/admin/departments?status=all")
    ...
    await expectError(
      await jsonRequest(admin, "/v1/admin/departments", {
        method: "POST",
        body: { name: "No Key" },
      }),
      422,
  ...
  it("停用：阻断三兄弟 / 503 可重试夹具 / 成功与重复停用", async () => {
    ...
    await expectError(
      await postWithKey(
        admin,
        "/v1/admin/departments/d_finance/deactivate",
        { expected_version: 1 },
        "idem_d_6",
      ),
      409,
    ...
    await expectError(
      await postWithKey(
        admin,
        "/v1/admin/departments/d_hr/deactivate",
        { expected_version: 1 },
        "idem_d_7",
      ),
      409,
...
describe("admin contract：备份与恢复（ops-only）", () => {
  it("非 ops 一律 403 backup_forbidden（admin / user 读与写同拒）", async () => {
    ...
    const user = bearerOf("zhangsan");
    await expectError(
      await jsonRequest(admin, "/v1/ops/backups"),
      403,
      "backup_forbidden",
    );
    await expectError(
      await jsonRequest(user, "/v1/ops/restores"),
      403,
      "backup_forbidden",
    );
    await expectError(
    ...
    await expectError(
      await jsonRequest(user, "/v1/ops/backups", {
        method: "POST",
        idempotencyKey: "idem_f_1",
      }),
      403,
      "backup_forbidden",
    );
  ...
  it("创建备份：202 受理 + 同键回放不重复创建 + 缺 Idempotency-Key 422", async () => {
    ...
    expect(replay.status).toBe(202);
    expect(((await replay.json()) as OpsBackupCreateResponse).backup_id).toBe(
      createdBody.backup_id,
    );
    const list = (await (
  ...
  it("维护门禁：种子 blocked 恢复 active 期间，新建备份与策略 PATCH 503；第二个恢复 409；GET 与 repair retry 白名单可用", async () => {
    ...
    await expectError(
      await jsonRequest(ops, "/v1/ops/backups", {
        method: "POST",
        idempotencyKey: "idem_g_1",
      }),
      503,
      "maintenance_mode",
    );
  ...
  it("创建恢复：202 + 七阶段形状；同键回放；异目标同键按新操作处理；不存在 / 不可恢复备份", async () => {
    ...
    await expectError(
      await postWithKey(
        ops,
        "/v1/ops/restores",
        { backup_id: freshBackup.backup_id },
        "idem_rs_4",
      ),
      409,
  ...
  it("repair retry：202 受理（open）；读推进两轮收敛终态；再 retry 409；未知目标 404；非空 body 422", async () => {
    ...
    const accepted = await jsonRequest(ops, retryPath, {
      method: "POST",
      idempotencyKey: "idem_rt_1",
    });
    expect(accepted.status).toBe(202);
    const acceptedBody =
      (await accepted.json()) as OpsRepairTargetRetryResponse;
    expect(acceptedBody).toEqual({
      target_id: OPS_BACKUP_SEED_IDS.repairTarget,
      status: "open",
    });
    // 同键回放：202 同形
    const replay = await jsonRequest(ops, retryPath, {
      method: "POST",
      idempotencyKey: "idem_rt_1",
    });
    expect(replay.status).toBe(202);
    expect(((await replay.json()) as OpsRepairTargetRetryResponse).status).toBe(
      "open",
    );
    ...
    const firstRead = (await (
      await jsonRequest(
        ops,
        `/v1/ops/restores/${OPS_BACKUP_SEED_IDS.repairRestore}`,
      )
    ).json()) as OpsRestoreDetail;
    ...
    await expectError(
      await jsonRequest(
        ops,
        "/v1/ops/restores/rs_missing/repair-targets/rt_x/retry",
        {
          method: "POST",
