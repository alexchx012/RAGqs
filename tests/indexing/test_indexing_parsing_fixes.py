...
# --- A4: provider_reconciling 契约（status 枚举 + worker 恢复流为权威实现） ---


def test_execution_status_constraint_includes_provider_reconciling() -> None:
    constraints = {
...
def test_worker_recovery_flow_uses_provider_reconciling_status() -> None:
    import inspect
    ...
    try:
        ...
        with engine.begin() as connection:
            ...
            status = connection.execute(
                text(
                    "SELECT status FROM chat_generation_execution " "WHERE execution_id = 'exec_1'"
                )
            ).scalar_one()
