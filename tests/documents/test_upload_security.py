...
def test_text_with_nul_bytes_is_rejected(service, principal) -> None:
    upload = DocumentUpload(
        filename="note.txt", content=b"head\x00tail", media_kind="text/plain"
    )
    with pytest.raises(PlatformError) as exc_info:
        service.create_upload(
...
def test_rejected_upload_leaves_no_documents_or_jobs(service, principal) -> None:
    ...
    with service._engine.connect() as connection:
        documents = connection.execute(select(func.count()).select_from(documents_table)).scalar_one()
        jobs = connection.execute(select(func.count()).select_from(ingestion_jobs_table)).scalar_one()
    assert documents == 0
    assert jobs == 0
...
def test_admin_viewing_foreign_personal_library_is_audited(service, principal) -> None:
    upload = DocumentUpload(
        filename="note.txt", content=b"personal note", media_kind="text/plain"
    )
