...
from sqlalchemy import Engine, and_, delete, exists, func, inspect, literal, select, text, update
...
class IdentityAccessService:
    ...
    @staticmethod
    def _documents_table_available(connection: Connection) -> bool:
        return inspect(connection).has_table(documents_table.name)

    def _document_count_for_space(self, connection: Connection, space_id: str) -> int:
        if not self._documents_table_available(connection):
            return 0
        return int(
            connection.execute(
                select(func.count())
    ...
    def _managed_user_response_for_id(
        ...
        response["document_count"] = self._document_count_for_space(
            connection, f"personal:{user_id}"
        )
        return response
        ...
        if isinstance(self._department_work_check, UnavailableDepartmentWorkCheckPort):
            ...
            try:
                work_state = self._department_work_check.inspect(
                    department_id, connection=connection
                )
            except RuntimeError:
                ...
                work_state = DepartmentWorkState()
        return {
            "document_count": self._document_count_for_space(
                connection, f"department:{department_id}"
            ),
    ...
    def list_managed_users(
        ...
        *,
        ...
        with self._engine.begin() as connection:
            ...
            if records and self._documents_table_available(connection):
                space_ids = [f"personal:{record[identity_user_table.c.id]}" for record in records]
                count_rows = connection.execute(
                    select(documents_table.c.space_id, func.count())
                    .where(
                        documents_table.c.space_id.in_(space_ids),
                        documents_table.c.lifecycle_status == "active",
                    )
                    .group_by(documents_table.c.space_id)
                ).all()
                document_counts = {
                    str(space_id).removeprefix("personal:"): int(count)
                    for space_id, count in count_rows
                }
        ...
        *,
        ...
        with self._engine.connect() as connection:
            ...
            for department in departments:
                ...
                counts = self._department_read_counts(connection, str(department["id"]))
                items.append(
                    {
                        "id": department["id"],
                        "name": department["name"],
